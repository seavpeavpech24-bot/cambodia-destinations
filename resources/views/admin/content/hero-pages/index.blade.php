...
@section('title', 'Hero Pages Management')
...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Hero Pages</h2>
            <p class="text-gray-600 mt-2 text-sm">Manage hero sections for different pages</p>
        </div>
        <a href="{{ route('admin.hero-pages.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700 transition duration-300 ease-in-out transform hover:-translate-y-1">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            ...
            <div>
                 {{-- Example filter for page - uncomment and populate $availablePages from controller if needed --}}
                 {{--
                 <select name="page" class="w-full sm:w-48 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
                    <option value="">All Pages</option>
                    @foreach($availablePages as $pageOption)
                        <option value="{{ $pageOption }}" {{ request('page') == $pageOption ? 'selected' : '' }}>{{ ucfirst($pageOption) }}</option>
                    @endforeach
                </select>
                --}}
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Description</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @forelse($heroPages as $heroPage)
                        <tr class="hover:bg-blue-50 transition duration-200">
                        <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $heroPage->id }}</div>
                            </td>
                             <td class="px-6 py-4">
                                @if($heroPage->image_url)
                                    <img src="{{ asset($heroPage->image_url) }}" alt="Hero Image" class="h-16 w-auto object-cover rounded">
                                @else
                                    N/A
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $heroPage->title }}</div>
                            </td>
                            <td class="px-6 py-4 hidden md:table-cell">
                                <div class="text-sm text-gray-600 max-w-xs truncate">{{ $heroPage->description ?? 'N/A' }}</div>
                            </td>
                             <td class="px-6 py-4">
                                <div class="text-sm text-gray-600">{{ $heroPage->page }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $heroPage->created_at->format('m-d-Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                     <a href="{{ route('admin.hero-pages.show', $heroPage) }}" class="text-blue-600 hover:text-blue-800 transition duration-200" title="View">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.hero-pages.edit', $heroPage) }}" class="text-green-600 hover:text-green-800 transition duration-200" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    {{-- Delete button removed as per user request
                                    <button onclick="confirmDelete({{ $heroPage->id }}, '{{ addslashes($heroPage->title) }}')" class="text-red-600 hover:text-red-800 transition duration-200" title="Delete">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {{-- Delete Confirmation Modal (Removed as per user request)
    <div id="deleteModal" class="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-xl p-6 w-full max-w-md transform transition-all duration-300 scale-95">
            <div class="text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900 mt-4">Delete Hero Page</h3>
    ...
    {{-- Inline JavaScript for Delete Modal (Removed as per user request)
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            window.confirmDelete = function (id, title) {
                document.getElementById('deleteItemName').textContent = title;
                document.getElementById('deleteForm').action = `{{ route('admin.hero-pages.destroy', ':id') }}`.replace(':id', id);
