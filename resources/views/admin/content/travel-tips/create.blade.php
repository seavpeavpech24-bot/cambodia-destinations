...
@section('title', 'Create Travel Tip')
...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Create Travel Tip</h2>
            <p class="text-gray-600 mt-2 text-sm">Add a new travel tip for a destination</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <form method="POST" action="{{ route('admin.travel-tips.store') }}">
            @csrf
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="group_by" id="group_by" class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
                    @error('group_by')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
