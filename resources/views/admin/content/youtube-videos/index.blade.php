...
@section('title', 'YouTube Videos Management')
...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">YouTube Videos</h2>
            <p class="text-gray-600 mt-2 text-sm">Manage YouTube video content</p>
        </div>
        <a href="{{ route('admin.youtube-videos.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700 transition duration-300 ease-in-out transform hover:-translate-y-1">
        ...
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <input type="text" 
                       ...
                       placeholder="Search videos by title or video ID..." 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Thumbnail</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Title</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Video ID</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($youtubeVideos as $video)
                        <tr class="hover:bg-indigo-50 transition duration-200">
                            <td class="px-6 py-4">
                                @if($video->video_id)
                                     <img src="https://img.youtube.com/vi/{{ $video->video_id }}/default.jpg" alt="{{ $video->title ?? 'YouTube Thumbnail' }}" class="w-24 h-14 object-cover rounded">
                                @else
                                    N/A
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $video->title ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 hidden md:table-cell">
                                <div class="text-sm text-gray-600">{{ $video->video_id ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 hidden lg:table-cell">
                                <div class="text-sm text-gray-600 max-w-xs truncate">{{ $video->description ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $video->created_at->format('m-d-Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                     <a href="{{ route('admin.youtube-videos.show', $video) }}" class="text-blue-600 hover:text-blue-800 transition duration-200" title="View">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.youtube-videos.edit', $video) }}" class="text-green-600 hover:text-green-800 transition duration-200" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-xl p-6 w-full max-w-md transform transition-all duration-300 scale-95">
            <div class="text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900 mt-4">Delete YouTube Video</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            window.confirmDelete = function (id, title) {
                ...
                document.getElementById('deleteForm').action = `/admin/youtube-videos/${id}`;
