...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Destinations</h2>
            <p class="text-gray-600 mt-2 text-sm">Manage your website's destination listings</p>
        </div>
        <a href="{{ route('admin.destinations.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700 transition duration-300 ease-in-out transform hover:-translate-y-1">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            ...
            <div>
                <select name="category_id" class="w-full sm:w-48 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Location</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Category</th>
                        ...
                        <tr class="hover:bg-indigo-50 transition duration-200">
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($destination->cover_url)
                                    <img src="{{ $destination->cover_url }}" alt="{{ $destination->title }} Cover" class="h-16 w-24 rounded-lg object-cover">
                                @else
                                    <div class="h-16 w-24 bg-gray-100 rounded-lg flex items-center justify-center">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $destination->title }}</div>
                            </td>
                            <td class="px-6 py-4 hidden md:table-cell">
                                <a href="{{ $destination->map_link ?? 'https://www.google.com/maps' }}" target="_blank"><div class="text-sm text-blue-600">{{ $destination->location ?? 'N/A' }}</div></a>
                            </td>
                            <td class="px-6 py-4 hidden lg:table-cell">
                                <div class="text-sm text-gray-600">{{ $destination->category->title ?? 'Uncategorized' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $destination->created_at->format('m-d-Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    @if($destination->google_map_link)
                                    <a href="{{ $destination->google_map_link }}" class="text-indigo-600 hover:text-indigo-800 transition duration-200" title="View on Map" target="_blank" rel="noopener noreferrer">
                                        <i class="fas fa-map-marker-alt w-5 h-5 text-red-600"></i>
                                    </a>
                                    @endif
                                    <a href="{{ route('admin.destinations.show', $destination) }}" class="text-indigo-600 hover:text-indigo-800 transition duration-200" title="View">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.destinations.edit', $destination) }}" class="text-green-600 hover:text-green-800 transition duration-200" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    <button type="button" onclick="confirmDelete({{ $destination->id }}, '{{ $destination->title }}')" class="text-red-600 hover:text-red-800 transition duration-200" title="Delete">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-xl p-6 w-full max-w-md transform transition-all duration-300 scale-95">
            <div class="text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900 mt-4">Delete Destination</h3>
                <div class="mt-2 px-4 py-3">
                    <p class="text-sm text-gray-600">
                        Are you sure you want to delete "<span id="deleteItemName" class="font-medium"></span>"? 
                        This action will soft delete the destination and may affect related tips and gallery items.
                    </p>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            window.confirmDelete = function (id, title) {
                ...
                document.getElementById('deleteForm').action = `/admin/destinations/${id}`;
