...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Create Destination Category</h2>
            <p class="text-gray-600 mt-2 text-sm">Add a new category for destinations</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.destination-categories.store') }}">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="icon_class" id="icon_class" value="{{ old('icon_class') }}" class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
                    @error('icon_class')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
