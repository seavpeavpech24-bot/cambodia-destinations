...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Gallery Item</h2>
            <p class="text-gray-600 mt-2 text-sm">Edit an existing gallery item</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.gallery.update', $gallery) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <img src="{{ asset($gallery->image_url) }}" alt="Current Gallery Image" class="h-32 w-auto object-cover rounded">
                    </div>
                </div>
                @endif
                ...
                <div>
                    ...
                    <input type="file" name="image" id="image" class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                    <p class="mt-1 text-sm text-gray-500">Upload a new image to replace the current one (max 2MB).</p>
                    @error('image')
                ...
                <div>
                    ...
                    <select name="destination_id" id="destination_id" class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                        ...
                        @foreach($destinations as $destination)
                            <option value="{{ $destination->id }}" {{ old('destination_id', $gallery->destination_id) == $destination->id ? 'selected' : '' }}>{{ $destination->title }}</option>
                        @endforeach
                    </select>
                ...
                <!-- Main Page Display Checkbox -->
                 <div class="flex items-center">
                    <input type="checkbox" name="main_page_display" id="main_page_display" value="1" {{ old('main_page_display', $gallery->main_page_display) ? 'checked' : '' }} class="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
