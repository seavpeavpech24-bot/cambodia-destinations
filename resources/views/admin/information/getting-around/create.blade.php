...
@section('title', 'Add New Getting Around Information')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Add New Getting Around Information</h2>
            <p class="text-gray-600 mt-2 text-sm">Create a new local transport or travel tip</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.getting-around.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                >
                    ...
                    @foreach(App\Models\GettingAround::GROUP_BY_OPTIONS as $value => $label)
                        <option value="{{ $value }}" {{ old('group_by') == $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
