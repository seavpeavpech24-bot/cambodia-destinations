...
@section('title', 'Add New Best Visiting Time')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Add New Best Visiting Time</h2>
            <p class="text-gray-600 mt-2 text-sm">Create a new recommendation for best visiting times</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.best-visiting-times.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                >
                    ...
                    @foreach(App\Models\BestVisitingTime::GROUP_BY_OPTIONS as $key => $value)
                        <option value="{{ $key }}" {{ old('group_by') == $key ? 'selected' : '' }}>
                            {{ $value }}
                        </option>
                    @endforeach
                </select>
