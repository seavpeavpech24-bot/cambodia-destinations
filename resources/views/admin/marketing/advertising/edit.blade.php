...
<div class="container mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Advertisement</h2>
            <p class="text-gray-600 mt-2 text-sm">Update details for the advertisement</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <!-- Ad Preview -->
        <div id="ad_preview" class="space-y-6 {{ $advertising->image_url || $advertising->video_url ? '' : 'hidden' }}">
            <div id="image_ad_preview" class="{{ $advertising->image_url ? '' : 'hidden' }} aspect-w-16 aspect-h-9">
                <img id="ad_preview_img" src="{{ $advertising->image_url ? Storage::url($advertising->image_url) : '' }}" alt="Ad Preview" class="w-full h-96 object-cover rounded-lg">
            </div>
            <div id="video_ad_preview" class="{{ $advertising->video_url ? '' : 'hidden' }} aspect-w-16 aspect-h-9">
                <video id="ad_preview_video" controls class="w-full h-96 rounded-lg">
                    <source id="ad_preview_video_source" src="{{ $advertising->video_url ? Storage::url($advertising->video_url) : '' }}" type="video/mp4">
                    Your browser does not support the video tag.
                </video>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.advertising.update', $advertising->id) }}" enctype="multipart/form-data" id="advertisementForm">
            @csrf
            @method('PUT')
            <div class="space-y-6 mt-6">
                ...
                <div>
                    ...
                    <div class="mt-1 flex gap-4">
                        <label class="inline-flex items-center">
                            <input type="radio" name="media_type" value="image" {{ old('media_type', $advertising->image_url ? 'image' : 'video') === 'image' ? 'checked' : '' }} class="form-radio text-indigo-600" onchange="toggleMediaInputs()">
                            <span class="ml-2 text-sm text-gray-600">Image</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="media_type" value="video" {{ old('media_type', $advertising->video_url ? 'video' : 'image') === 'video' ? 'checked' : '' }} class="form-radio text-indigo-600" onchange="toggleMediaInputs()">
                ...
                <!-- Image Upload -->
                <div id="image_input" class="{{ old('media_type', $advertising->image_url ? 'image' : 'video') === 'image' ? 'block' : 'hidden' }}">
                    ...
                    <div class="mt-1">
                        ...
                        @if($advertising->image_url)
                            <p class="mt-1 text-sm text-gray-600">Current: <a href="{{ Storage::url($advertising->image_url) }}" target="_blank" class="text-indigo-600 hover:underline">View Image</a></p>
                        @endif
                    </div>
                    @error('image')
                ...
                <!-- Video Upload -->
                <div id="video_input" class="{{ old('media_type', $advertising->video_url ? 'video' : 'image') === 'video' ? 'block' : 'hidden' }}">
                    <label for="video" class="block text-sm font-medium text-gray-700">Upload New Video (Optional)</label>
                    <div class="mt-1">
                        <input type="file" name="video" id="video" accept="video/*" class="hidden" onchange="handleVideoUpload(this)">
                        ...
                        @if($advertising->video_url)
                            <p class="mt-1 text-sm text-gray-600">Current: <a href="{{ Storage::url($advertising->video_url) }}" target="_blank" class="text-indigo-600 hover:underline">View Video</a></p>
                        @endif
                    </div>
                    @error('video')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $advertising->title) }}" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200" placeholder="Enter ad title...">
                    @error('title')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200" placeholder="Enter ad description...">{{ old('description', $advertising->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="url" name="link" id="link" value="{{ old('link', $advertising->link) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200" placeholder="Enter ad link (e.g., https://example.com)">
                    @error('link')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $advertising->start_date->format('Y-m-d')) }}" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
                    @error('start_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="date" name="expire_date" id="expire_date" value="{{ old('expire_date', $advertising->expire_date->format('Y-m-d')) }}" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
                    @error('expire_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="mt-1 flex gap-4">
                        ...
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_visible" value="1" 
                                {{ (old('is_visible') !== null ? old('is_visible') : $advertising->is_visible) ? 'checked' : '' }} 
    ...
    <script>
        ...
        function toggleMediaInputs() {
            ...
            if (mediaType === 'image') {
                ...
                if (document.getElementById('image').files[0] || '{{ $advertising->image_url }}') {
                    imageAdPreview.classList.remove('hidden');
                ...
                if (document.getElementById('video').files[0] || '{{ $advertising->video_url }}') {
                    videoAdPreview.classList.remove('hidden');
                    adPreview.classList.remove('hidden');
                }
            }
            // Hide ad preview if no file is selected and no existing media
            if (!document.getElementById('image').files[0] && !document.getElementById('video').files[0] && !'{{ $advertising->image_url }}' && !'{{ $advertising->video_url }}') {
        ...
        function handleVideoUpload(input) {
            ...
            uploadTimeout = setTimeout(() => {
                ...
                if (file) {
                    ...
                    const progressText = document.getElementById('video_progress_text');
                    progressBar.classList.remove('hidden');
                    progressText.classList.remove('hidden');
                    let progress = 0;
                    const interval = setInterval(() => {
        ...
        function removeImage() {
            ...
            // Reset to current image if exists
            if ('{{ $advertising->image_url }}') {
                document.getElementById('ad_preview_img').src = '{{ Storage::url($advertising->image_url) }}';
                document.getElementById('image_ad_preview').classList.remove('hidden');
                adPreview.classList.remove('hidden');
        ...
        function removeVideo() {
            ...
            const progressBar = document.getElementById('video_progress');
            const progressText = document.getElementById('video_progress_text');
            const removeButton = document.getElementById('video_remove_button');
            input.value = '';
            document.getElementById('ad_preview_video_source').src = '';
            document.getElementById('ad_preview_video').load();
            adPreview.classList.add('hidden');
            progressBar.classList.add('hidden');
            progressText.classList.add('hidden');
            progressBar.value = 0;
            progressText.textContent = '0%';
            removeButton.classList.add('hidden');
            // Reset to current video if exists
            if ('{{ $advertising->video_url }}') {
                document.getElementById('ad_preview_video_source').src = '{{ Storage::url($advertising->video_url) }}';
                document.getElementById('ad_preview_video').load();
                document.getElementById('video_ad_preview').classList.remove('hidden');
                adPreview.classList.remove('hidden');
                removeButton.classList.remove('hidden');
        ...
        document.getElementById('advertisementForm').addEventListener('submit', function(e) {
            ...
            // Show progress bar if a new file is being uploaded
            if ((mediaType === 'image' && imageInput.files[0]) || (mediaType === 'video' && videoInput.files[0])) {
                progressBar.classList.remove('hidden');
                progressText.classList.remove('hidden');
            }
        ...
        // Initialize media input visibility and remove buttons
        window.onload = function() {
            toggleMediaInputs();
            if ('{{ $advertising->image_url }}' && document.querySelector('input[name="media_type"]:checked').value === 'image') {
                document.getElementById('image_remove_button').classList.remove('hidden');
            } else if ('{{ $advertising->video_url }}' && document.querySelector('input[name="media_type"]:checked').value === 'video') {
                document.getElementById('video_remove_button').classList.remove('hidden');
            }
        };
    </script>
